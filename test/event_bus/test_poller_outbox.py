...
@pytest.mark.asyncio(loop_scope="session")
async def test_publishes_event_id_in_payload(kafka_producer, poller, create_drop_tables):
    e = _make_event()
    async with get_db_session() as db_sess:
        db_sess.add(e)
...
@pytest.mark.asyncio(loop_scope="session")
async def test_multiple_events_all_published(kafka_producer, poller, create_drop_tables):
...
@pytest.mark.asyncio(loop_scope="session")
async def test_skips_completed_publishes_pending(kafka_producer, poller, create_drop_tables):
    pending = _make_event()
    ...
    async with get_db_session() as db_sess:
        assert (await db_sess.get(EventOutbox, pending.id)).status == EventStatus.COMPLETED
        assert (await db_sess.get(EventOutbox, completed.id)).status == EventStatus.COMPLETED


@pytest.mark.asyncio(loop_scope="session")
async def test_failed_reprocessed_and_published(kafka_producer, poller, create_drop_tables):
