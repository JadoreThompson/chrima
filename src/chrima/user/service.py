...
class UserService:
    ...
    async def get_user_by_id(self, user_id: UUID, db_sess: AsyncSession) -> User:
        user = await db_sess.get(User, user_id)
        if user is None:
            raise UserNotFoundException()
        return user

    async def get_user(self, user_id: UUID, db_sess: AsyncSession) -> UserResponse:
        user = await self.get_user_by_id(user_id, db_sess)
        return UserResponse(
            id=user.id,
    ...
    async def find_user(self, email: str, db_sess: AsyncSession) -> User:
