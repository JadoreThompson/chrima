...
class ProductSyncService:
    ...
    async def run(self):
        self._logger.info("Starting ProductSyncService (interval=%ss)", self._interval)
        ...
        try:
            ...
            async for msg in consumer:
                ...
                if event.type == ProductEventType.WALLET_UPDATED:
                    await self._handle_wallet_updated(event)
        finally:
            await consumer.stop()

    async def _handle_wallet_updated(self, event: ProductWalletUpdatedEvent) -> None:
            ...
            for attempt in range(3):
                try:
                    ...
                    self._logger.info(
                        ...
                        tx_hash.hex(), event.product_id, wallet.wallet_address, receipt["status"],
                    )
                    return
